AFRAME.registerComponent("atoms", {
  ...
  createAtoms: async function (element) {
    ...
    for(var num = 1; num<=numOfElectron;num++){
      ...
      electronGroup.setAttribute("id", `election-group-${elementName}`)
      ...
      electron.setAttribute("id", `election-${elementName}`);
      electron.setAttribute("geoemtry", {
        primitive:"sphere",
        radius:0.02,
      })
      ...
      electron.setAttribute("postion", {x:0.2, y:0.2, z:0});
